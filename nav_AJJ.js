...
class mainNav {
    ...
    init() {
        ...
        _this.topIcon.addEventListener("click", function () {
            ...
            // _this.mainHeaderTag.style.height = "8vh";
            _this.navEvent(0);
        ...
        _this.navChildTags.forEach((el, idx) => {
            el.onclick = () => {
                ...
                // _this.mainHeaderTag.style.height = "16vh";
                setTimeout(() => {
        ...
        // 창이 resize 되면 section의 크기가 달라지기 때문에 sectionTags의 top을 0으로 초기화시키고 마지막으로 보았던 해당 section[index] 영역으로 스크롤을 이동시킨다.
        window.onresize = () => {
            _this.navEvent(_this.index);
            _this.remoconReMove(_this.theTop);
            // ㅗ idx+1 값을 가져오기 위해 사용한다.
        // ㅗ 마지막으로 보았던 section[index] 정보가 들어있다.
        // ㅗ onclick 안 하면 index 초기 값이 0이기 때문에 메인 페이지가 그대로 resize 된다.
        };
    }
    ...
    navEvent(idx) {
        ...
        const sectionTagsTop = _this.whereIsSectionTop(_this.theTop);
        _this.theTop = sectionTagsTop[idx]; // 클릭한 메뉴의 해당 section의 top이 들어있다.
        allSectionsTag.style.top = `-${_this.theTop}px`; // 스크롤 이동
    }

    // sectionTags의 top을 sectionTagsTop 배열에 담아 return하는 함수
    whereIsSectionTop(theTop) {
        ...
        for (let i = 0; i < _this.sectionTags.length; i++) {
            ...
            // ㅗ sectionTags[i].getBoundingClientRect().top 값과 theTop 값은 마이너스를 제외하고 동일하다.
            // ㅗ 따라서 theTop을 더해줌으로써 section1의 top을 0으로 만든다.
        }
